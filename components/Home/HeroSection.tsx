...
export default function HeroSection() {
  ...
  const skills = useMemo(
    () => ["Next.js", "React", "UI/UX", "TypeScript", "Full Stack"],
    []
  );
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      setTypedText((prev) =>
        isDeleting ? prev.slice(0, -1) : currentSkill.slice(0, prev.length + 1)
      );
    }, typeSpeed);
  ...
  return (
    <section
      className="w-full min-h-[80vh] flex items-center py-12 xl:flex-row flex-col space-y-6 z-10"
      id="hero"
    >
      <div className="w-full space-y-8 text-center xl:text-left px-6">
        ...
        <h1 className="text-6xl font-black leading-tight">{t("Title")}</h1>

        <p className="text-4xl font-light">{t("Subtitle")}</p>
