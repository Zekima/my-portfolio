...
import { X, ChevronLeft, ChevronRight, ZoomIn, ZoomOut, Info, Download, Share } from 'lucide-react';
...
export default function ImageGallery({
  ...
  const imageData: ImageData[] = images.map(img => 
    typeof img === 'string' ? { filename: img } : img
  );
  ...
  const allTags = Array.from(
    new Set(imageData.flatMap(img => img.tags || []))
  );
          ...
          >
            ...
            >
              <Image
                ...
                alt={image.title || image.filename.replace(/\.(png|jpg|jpeg|gif|webp)$/i, '')}
                fill
              ...
              {(image.title || image.description) && (
                <div className="absolute inset-x-0 bottom-0 bg-black/70 text-white p-3 translate-y-full group-hover:translate-y-0 transition-transform">
                  {image.title && <h3 className="font-medium text-sm">{image.title}</h3>}
                  {image.description && (
        ...
        <div className="fixed inset-0  top-0 bg-black/90 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-4">
          ...
          <div className="absolute top-0 left-0 right-0 flex justify-between items-center p-4 bg-black/50 z-10">
            <div className="text-white">
              {filteredImages[selectedIndex].title || filteredImages[selectedIndex].filename.replace(/\.(png|jpg|jpeg|gif|webp)$/i, '')}
            </div>
          ...
          >
            ...
            >
              <Image
                ...
                alt={filteredImages[selectedIndex].title || filteredImages[selectedIndex].filename.replace(/\.(png|jpg|jpeg|gif|webp)$/i, '')}
                width={1920}
                height={1080}
          ...
          {showInfo && (
            <div className="absolute right-0 top-16 bottom-0 w-80 bg-black/80 p-6 overflow-y-auto text-white">
              ...
              {filteredImages[selectedIndex].description && (
                <p className="text-sm text-gray-200 mb-4">{filteredImages[selectedIndex].description}</p>
              )}
              {filteredImages[selectedIndex].tags && filteredImages[selectedIndex].tags.length > 0 && (
                <div className="mt-4">
              ...
              <div className="mt-4">
                ...
                <div className="text-xs space-y-1">
                  ...
                  <p>Image {selectedIndex + 1} of {filteredImages.length}</p>
                </div>
              </div>
            </div>
          )}
                ...
                {filteredImages.map((img, idx) => (
                  <button
                    ...
                    className={`relative flex-shrink-0 w-20 h-14 rounded overflow-hidden transition-all ${
                      selectedIndex === idx ? 'ring-2 ring-primary scale-105' : 'opacity-60 hover:opacity-100'
                    }`}
                  >
                    <Image
                      src={`/images/projects/${projectName}/gallery/${img.filename}`}
                      alt={img.title || img.filename.replace(/\.(png|jpg|jpeg|gif|webp)$/i, '')}
                      fill
